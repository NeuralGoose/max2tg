...
| `/mute` | Заглушить/включить уведомления для темы — отправьте **внутри неё**. Каналы по умолчанию приходят без звука; `/mute` переключает любой чат. |
| `/help` · `/start` | Справка и приветствие. |

> 🔔 В каждой теме сверху **закреплена кнопка** «🔕 Выключить звук / 🔔 Включить звук» — тапните, чтобы переключить уведомления этого чата (как кнопка «Вкл. звук» у каналов). То же делает `/mute`.
